...
class TestDocumentStorageService:
    ...
    async def test_download_file_from_url_success(self):
        ...
        with patch.object(service, "download_file_from_url") as mock_download:
            ...
            content, filename, content_type = await service.download_file_from_url(
    ...
    async def test_download_file_from_url_http_error(self):
        ...
        with patch.object(service, "download_file_from_url") as mock_download:
            mock_download.side_effect = DownloadError(
                "HTTP 404: Failed to download from https://example.com/notfound.jpg"
            )
    ...
    async def test_download_file_from_url_invalid_content_type(self):
        ...
        with patch.object(service, "download_file_from_url") as mock_download:
            mock_download.side_effect = FileValidationError(
    ...
    async def test_download_file_from_url_file_too_large(self):
        ...
        with patch.object(service, "download_file_from_url") as mock_download:
            mock_download.side_effect = FileValidationError(
                f"File too large: {MAX_FILE_SIZE + 1} bytes"
            )
    ...
    async def test_download_file_network_error(self):
        ...
        with patch.object(service, "download_file_from_url") as mock_download:
            mock_download.side_effect = DownloadError(
                "Network error downloading from https://example.com/test.jpg: Connection failed"
            )
    ...
    async def test_read_local_file_success(self):
        ...
        with patch("app.services.document_storage.Path") as mock_path_class:
            ...
            with patch.object(service, "_validate_file_extension", return_value=True):
                with patch(
                    "app.services.document_storage.mimetypes.guess_type"
                ) as mock_guess_type:
                    mock_guess_type.return_value = ("image/jpeg", None)

                    with patch(
                        "app.services.document_storage.aiofiles.open"
                    ) as mock_aiofiles_open:
                        mock_file = AsyncMock()
                        mock_file.read.return_value = file_content
                        mock_aiofiles_open.return_value.__aenter__.return_value = (
                            mock_file
                        )
                        mock_aiofiles_open.return_value.__aexit__.return_value = None

                        content, filename, content_type = await service.read_local_file(
                            file_path
                        )
    ...
    async def test_process_url_upload_success(self):
        ...
        with patch.object(service, "download_file_from_url") as mock_download:
            ...
            with patch.object(service, "upload_to_storage") as mock_upload:
                ...
                with patch.object(
                    service, "_generate_storage_path"
                ) as mock_generate_path:
                    mock_generate_path.return_value = (
                        f"{driver_id}/CDL/20240115_120000_image.jpg"
                    )

                    result = await service.process_url_upload(
                        media_url, driver_id, "CDL"
                    )
        ...
        with patch.object(service, "read_local_file") as mock_read:
            ...
            with patch.object(service, "upload_to_storage") as mock_upload:
                ...
                with patch.object(
                    ...
                    result = await service.process_local_upload(
                        file_path, driver_id, "CDL"
                    )
