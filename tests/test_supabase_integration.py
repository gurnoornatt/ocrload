...
async def test_existing_tables_accessible():
    ...
    try:
        ...
        result = (
            supabase_service.client.table("drivers").select("id").limit(1).execute()
        )
    ...
    try:
        result = (
            supabase_service.client.table("documents").select("id").limit(1).execute()
        )
        assert result is not None
    ...
    try:
        result = (
            supabase_service.client.table("transactions")
            .select("id")
            .limit(1)
            .execute()
        )
...
async def test_storage_bucket_access():
    ...
    try:
        ...
        if settings.s3_bucket in bucket_names:
            ...
            print(
                f"Target bucket '{settings.s3_bucket}' not found, available: {bucket_names}"
            )
...
async def test_database_schema_structure():
    ...
    try:
        result = (
            supabase_service.client.table("drivers")
            .select("id, phone_number, doc_flags, status")
            .limit(1)
            .execute()
        )
        assert result is not None
    ...
    try:
        result = (
            supabase_service.client.table("loads")
            .select("id, origin, destination, rate, assigned_driver_id, status")
            .limit(1)
            .execute()
        )
        assert result is not None
