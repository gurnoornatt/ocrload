...
class TestRedisEventService:
    ...
    def test_client_property_successful_connection(
        self, mock_from_url, mock_settings, service, mock_redis_client
    ):
    ...
    def test_client_property_connection_failure(
        self, mock_from_url, mock_settings, service
    ):
    ...
    @pytest.mark.asyncio
    @patch("app.services.redis_event_service.settings")
    async def test_emit_invoice_ready_event_success(
        self, mock_settings, service, mock_redis_client
    ):
    ...
    @pytest.mark.asyncio
    @patch("app.services.redis_event_service.settings")
    async def test_emit_invoice_ready_event_with_additional_data(
        self, mock_settings, service, mock_redis_client
    ):
        ...
        result = await service.emit_invoice_ready_event(
            load_id, driver_id, additional_data
        )
    ...
    @pytest.mark.asyncio
    @patch("app.services.redis_event_service.settings")
    async def test_emit_invoice_ready_event_redis_error(
        self, mock_settings, service, mock_redis_client
    ):
    ...
    @pytest.mark.asyncio
    @patch("app.services.redis_event_service.settings")
    async def test_emit_custom_event_success(
        self, mock_settings, service, mock_redis_client
    ):
    ...
    def test_should_retry_connection_recent_failure(self, service):
        ...
        service._connection_failed = True
        service._last_connection_attempt = datetime.now(UTC)
    ...
    def test_should_retry_connection_old_failure(self, service):
        ...
        past_time = datetime.now(UTC).timestamp() - 600
        service._last_connection_attempt = datetime.fromtimestamp(
            past_time, UTC
        )
    ...
    @pytest.mark.asyncio
    async def test_emit_invoice_ready_event_retry_connection(
        ...
        service._connection_failed = True
        past_time = datetime.now(UTC).timestamp() - 600  # 10 minutes ago
        service._last_connection_attempt = datetime.fromtimestamp(
            past_time, UTC
        )
        ...
        with patch.object(
            type(service), "client", new_callable=PropertyMock
        ) as mock_client_prop:
