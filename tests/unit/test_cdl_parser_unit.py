...
class TestCDLParserMethods(unittest.TestCase):
    ...
    def test_clean_name_edge_cases(self):
        ...
        self.assertEqual(
            cleaned, "LICENSE CDL EXPIRES"
        )  # Returns original if all filtered
    ...
    def test_calculate_confidence(self):
        ...
        cdl_data = CDLData(
            driver_name="John Smith", license_number="D1234567", license_class="A"
        )
        confidence = self.parser._calculate_confidence(cdl_data, {})
        ...
        cdl_data = CDLData(license_class="A", address="123 Main St")
        confidence = self.parser._calculate_confidence(cdl_data, {})
    ...
    def test_is_cdl_verified(self):
        ...
        cdl_data = CDLData(driver_name="John Smith", expiration_date=future_date)
        self.assertTrue(self.parser._is_cdl_verified(cdl_data))

        # CDL expiring soon
        soon_date = datetime.now() + timedelta(days=20)
        cdl_data = CDLData(driver_name="John Smith", expiration_date=soon_date)
        self.assertFalse(self.parser._is_cdl_verified(cdl_data))

        # Missing name
        cdl_data = CDLData(expiration_date=future_date)
        self.assertFalse(self.parser._is_cdl_verified(cdl_data))

        # Missing expiration
        cdl_data = CDLData(driver_name="John Smith")
        self.assertFalse(self.parser._is_cdl_verified(cdl_data))
