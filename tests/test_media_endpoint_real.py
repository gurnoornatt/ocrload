...
class TestMediaEndpointRealIntegration:
    ...
    def test_media_upload_endpoint_accepts_request(
        self, client, test_driver_id, sample_image_url
    ):
    ...
    def test_media_upload_with_load_id(
        self, client, test_driver_id, test_load_id, sample_image_url
    ):
    ...
    async def test_complete_pipeline_with_real_services(
        ...
        with patch(
            "app.services.ocr_clients.unified_ocr_client.UnifiedOCRClient.process_file_content"
        ) as mock_ocr, patch(
            "app.services.document_parsers.cdl_parser.CDLParser.parse"
        ) as mock_parser:
            # Configure mocks
    ...
    async def test_large_file_rejection(self, client, test_driver_id):
        ...
        large_file_url = (
            "https://httpbin.org/bytes/20971520"  # 20MB, over our 10MB limit
        )
    ...
    async def test_different_document_types(
        ...
        with patch(
            ...
            "app.services.document_parsers.coi_parser.COIParser.parse"
        ) as mock_coi, patch(
            "app.services.document_parsers.agreement_parser.AgreementParser.parse"
        ) as mock_agreement, patch(
            "app.services.document_parsers.rate_confirmation_parser.RateConfirmationParser.parse"
        ) as mock_rate, patch(
            "app.services.document_parsers.pod_parser.PODParser.parse"
        ) as mock_pod:
            # Configure mocks
    ...
    @pytest.mark.asyncio
    async def test_redis_event_emission(
        self, client, test_driver_id, test_load_id, sample_image_url
    ):
        ...
        with patch(
            ...
            try:
                ...
                test_result = await redis_event_service.emit_invoice_ready(
                    driver_id=test_driver_id, load_id=test_load_id
                )
    ...
    async def test_local_fake_cdl_with_matching_driver(self, client):
        ...
        if status_data["status"] == "parsed":
            result = status_data["result"]
            print("✅ OCR SUCCESS! Extracted data:")
        ...
        assert document.type == DocumentType.CDL
        assert (
            str(document.driver_id) == driver_id
    ...
    async def test_real_cdl_document_processing(self, client, test_driver_id):
        ...
        print("   ✅ Real document: California CDL")
