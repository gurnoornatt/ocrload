    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            async with session.post(
                "https://www.datalab.to/api/v1/marker", data=form_data, headers=headers
            ) as response:
                        ...
                        if status == "complete":
                            ...
                            if "json" in poll_data:
                                ...
                                if isinstance(poll_data["json"], dict):
                                    print(
                                        f"JSON Keys: {list(poll_data['json'].keys())}"
                                    )
            ...
            try:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        submit_data = await response.json()
                        print(
                            f"✅ Submitted {output_format}: {submit_data.get('success')}"
                        )
                    else:
                        print(
                            f"❌ Submit failed for {output_format}: {response.status}"
                        )
...
async def main():
    ...
    if result:
        ...
        print(
            "💡 Ready to implement MarkerOCRClient with real understanding of API behavior"
        )
    else:
