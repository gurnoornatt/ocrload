...
class TestDatabaseFlagServiceReal:
    ...
    async def test_driver_verification_status_retrieval(self):
        ...
        try:
            ...
            try:
                result = await self.service.get_driver_verification_status(mock_driver_id)
    ...
    async def test_comprehensive_document_processing_flow(self):
        ...
        with patch.object(self.service.supabase, 'update_driver_flags') as mock_driver_flags, \
            ...
            for doc_config in test_documents:
                ...
                logger.info(f"Processed {doc_config['type']}: {len(result['flags_updated'])} flags updated")
            
            # Validate all document types were processed successfully
            assert len(results) == 5
            
            # Validate specific outcomes
            cdl_result, coi_result, agreement_result, ratecon_result, pod_result = results
    ...
    async def test_error_resilience(self):
        ...
        with patch.object(self.service.supabase, 'update_driver_flags') as mock_update:
            ...
            assert any("CDL expiry date not found" in rule for rule in result["business_rules_applied"])
    ...
    async def test_edge_case_scenarios(self):
        ...
        boundary_expiry = datetime.now(timezone.utc) + timedelta(days=30)
