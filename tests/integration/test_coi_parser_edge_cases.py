...
class TestCOIParserEdgeCases:
    ...
    def test_insurance_company_edge_cases(self):
        ...
        real_company_cases = [
            ...
            ("Government Employees Insurance Company", "Government Employees Insurance Company"),
            ("State Farm Fire and Casualty Company", "State Farm Fire And Casualty Company"),
            ("Travelers Property Casualty Company of America", "Travelers Property Casualty Company Of America"),
        ...
        cleaning_cases = [
            ("Progressive Commercial Insurance", "Progressive Commercial Insurance"),
            ("CERTIFICATE INSURANCE", "Insurance"),  # Should filter CERTIFICATE but keep Insurance
            ("Liberty Mutual Insurance Company", "Liberty Mutual Insurance Company"),
        ]
        
        for text, expected in cleaning_cases:
            result = self.parser._clean_company_name(text)
            assert result == expected, f"Cleaning failed for '{text}': expected '{expected}', got '{result}'"
    ...
    def test_currency_parsing_edge_cases(self):
        ...
        currency_cases = [
            ...
            ("1,000,000", "General Liability: $1,000,000", 100000000),  # Full number format
        ]
        
        for amount_str, full_match, expected in currency_cases:
            result = self.parser._parse_currency_amount(amount_str, full_match)
            assert result == expected, f"Currency parsing failed for '{amount_str}' in '{full_match}': expected {expected}, got {result}"
    ...
    def test_general_liability_extraction_edge_cases(self):
        ...
        for text, expected in gl_cases:
            ...
            assert result == expected, f"GL extraction failed for '{text}': expected {expected}, got {result}"
    ...
    def test_ocr_character_substitution_errors(self):
        ...
        ocr_error_cases = [
            ...
            ("Progressive Commercial lnsurance", "Progressive Commercial"),  # 'I' → 'l' substitution
        ]
        ...
        for text, expected in ocr_error_cases[:2]:
            details = {}
            result = self.parser._extract_policy_number(text, details)
            assert result == expected, f"OCR error handling failed for '{text}': expected {expected}, got {result}"
        
        # Test GL extraction with OCR errors  
        details = {}
        result = self.parser._extract_general_liability_amount(ocr_error_cases[2][0], details)
        assert result == ocr_error_cases[2][1], f"OCR GL extraction failed"
    ...
    def test_minimal_format_documents(self):
        ...
        gl_amount = self.parser._extract_general_liability_amount(minimal_coi_text, details)
        ...
        assert company is not None, f"Minimal format company extraction failed: {company}"
        assert gl_amount == 100000000, f"Minimal format GL extraction failed: {gl_amount}"  # $1M = 100000000 cents
        assert exp_date is not None, f"Minimal format expiration extraction failed: {exp_date}"
    ...
    def test_complex_monetary_formats(self):
        ...
        for text, expected in complex_money_cases:
            ...
            result = self.parser._extract_general_liability_amount(text, details) or \
                    self.parser._extract_auto_liability_amount(text, details)
            assert result == expected, f"Complex money format failed for '{text}': expected {expected}, got {result}"
    ...
    def test_poor_ocr_spacing(self):
        ...
        details = {}
        policy = self.parser._extract_policy_number(poor_spacing_text, details)
        gl_amount = self.parser._extract_general_liability_amount(poor_spacing_text, details)
    ...
    def test_mixed_case_text(self):
        ...
        assert result.data.policy_number is not None, "Should extract policy despite mixed case"
        assert result.data.insurance_company is not None, "Should extract company despite mixed case"
        assert result.data.general_liability_amount is not None, "Should extract GL amount despite mixed case"
        assert result.data.auto_liability_amount is not None, "Should extract auto amount despite mixed case"
        assert result.data.expiration_date is not None, "Should extract expiration despite mixed case"
        assert result.confidence > 0.8, f"Should have high confidence for complete data: {result.confidence}"
