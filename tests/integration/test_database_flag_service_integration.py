...
from app.services.database_flag_service import DatabaseFlagUpdateService, database_flag_service
from app.models.database import (
    Document, DocumentType, LoadStatus, DriverStatus,
...
class TestDatabaseFlagServiceIntegration:
    ...
    async def test_cdl_flag_update_success(self):
        ...
        # Arrange
        future_expiry = datetime.now(timezone.utc) + timedelta(days=60)  # 60 days from now
        cdl_data = {
        ...
        with patch.object(self.service.supabase, 'update_driver_flags') as mock_update_flags:
            ...
            assert any("Confidence threshold met" in rule for rule in result["business_rules_applied"])
            assert any("CDL expiry valid" in rule for rule in result["business_rules_applied"])
    ...
    async def test_cdl_flag_update_expiry_too_soon(self):
        ...
        soon_expiry = datetime.now(timezone.utc) + timedelta(days=20)
        cdl_data = {
            "driver_name": "John Smith",
            "license_number": "D123456789",
        ...
        with patch.object(self.service.supabase, 'update_driver_flags') as mock_update_flags:
            ...
            assert any("CDL expiry too soon" in rule for rule in result["business_rules_applied"])
    ...
    async def test_coi_flag_update_success(self):
        ...
        # Arrange
        future_expiry = datetime.now(timezone.utc) + timedelta(days=180)  # 6 months from now
        coi_data = {
        ...
        with patch.object(self.service.supabase, 'update_driver_flags') as mock_update_flags:
            ...
            assert any("COI not expired" in rule for rule in result["business_rules_applied"])
    ...
    async def test_coi_flag_update_expired(self):
        ...
        with patch.object(self.service.supabase, 'update_driver_flags') as mock_update_flags:
            ...
            assert any("COI expired" in rule for rule in result["business_rules_applied"])
    ...
    async def test_agreement_flag_update_with_signature(self):
        ...
        with patch.object(self.service.supabase, 'update_driver_flags') as mock_update_flags:
            ...
            assert any("Signature detected" in rule for rule in result["business_rules_applied"])
    ...
    async def test_agreement_flag_update_no_signature(self):
        ...
        with patch.object(self.service.supabase, 'update_driver_flags') as mock_update_flags:
            ...
            # Assert
            assert result["flags_updated"]["agreement_signed"] is True
            assert any("no signature detection" in rule for rule in result["business_rules_applied"])
    ...
    async def test_ratecon_flag_update_success(self):
        ...
        ratecon_data = {
            ...
            "pickup_date": datetime.now(timezone.utc) + timedelta(days=1),
        ...
        # Mock the supabase service
        with patch.object(self.service, '_update_load_ratecon_verified') as mock_update_ratecon:
            mock_update_ratecon.return_value = None
            
            # Act
            result = await self.service.process_document_flags(
                document, ratecon_data, confidence=0.85  # Lower confidence OK for rate cons
            )
            
            # Assert
            assert result["flags_updated"]["ratecon_verified"] is True
            assert any("All required fields present" in rule for rule in result["business_rules_applied"])
    ...
    async def test_ratecon_flag_update_missing_fields(self):
        ...
        with patch.object(self.service, '_update_load_ratecon_verified') as mock_update_ratecon:
            ...
            assert any("Missing required fields" in rule for rule in result["business_rules_applied"])
    ...
    async def test_pod_flag_update_success(self):
        ...
        # Mock the supabase service
        with patch.object(self.service.supabase, 'update_load_status') as mock_update_status, \
             patch.object(self.service, '_check_invoice_readiness') as mock_check_invoice:
            ...
            assert any("Delivery confirmed" in rule for rule in result["business_rules_applied"])
    ...
    async def test_pod_flag_update_no_delivery_confirmation(self):
        ...
        with patch.object(self.service.supabase, 'update_load_status') as mock_update_status:
            ...
            assert any("Delivery not confirmed" in rule for rule in result["business_rules_applied"])
    ...
    async def test_invoice_readiness_check(self):
        ...
        # Mock the supabase service
        with patch.object(self.service.supabase, 'update_load_status') as mock_update_status, \
             patch.object(self.service.supabase, 'check_load_ratecon_verified') as mock_check_ratecon:
            mock_update_status.return_value = True
            ...
            assert any("Invoice ready" in rule for rule in result["business_rules_applied"])
    
    @pytest.mark.asyncio
    async def test_low_confidence_scenarios(self):
        """Test that low confidence prevents flag updates across all document types."""
        test_cases = [
            (DocumentType.CDL, {"expiration_date": datetime.now(timezone.utc) + timedelta(days=60)}, self.mock_driver_id, None),
            (DocumentType.COI, {"expiration_date": datetime.now(timezone.utc) + timedelta(days=60)}, self.mock_driver_id, None),
            (DocumentType.AGREEMENT, {"signature_detected": True}, self.mock_driver_id, None),
        ...
        for doc_type, data, driver_id, load_id in test_cases:
            ...
            with patch.object(self.service.supabase, 'update_driver_flags') as mock_driver_flags, \
                 patch.object(self.service.supabase, 'update_load_status') as mock_load_status, \
                 patch.object(self.service, '_update_load_ratecon_verified') as mock_ratecon:
                ...
                assert any("Confidence too low" in rule for rule in result["business_rules_applied"])
    ...
    async def test_ratecon_low_confidence_but_complete_fields(self):
        ...
        with patch.object(self.service, '_update_load_ratecon_verified') as mock_update_ratecon:
            ...
            result = await self.service.process_document_flags(
                document, ratecon_data, confidence=0.6  # Below normal threshold
            )
    ...
    async def test_error_handling_missing_document_ids(self):
        ...
        driver_test_cases = [
            (DocumentType.CDL, {"expiration_date": datetime.now(timezone.utc) + timedelta(days=60)}),
            (DocumentType.COI, {"expiration_date": datetime.now(timezone.utc) + timedelta(days=60)}),
            (DocumentType.AGREEMENT, {"signature_detected": True}),
        ]
        
        load_test_cases = [
            (DocumentType.RATE_CON, {"rate_amount": 250000, "origin": "A", "destination": "B"}),
    ...
    async def test_get_driver_verification_status(self):
        ...
        mock_driver.updated_at = datetime.now(timezone.utc)
        
        with patch.object(self.service.supabase, 'get_driver_by_id') as mock_get_driver:
            mock_get_driver.return_value = mock_driver
            
            # Act
            result = await self.service.get_driver_verification_status(self.mock_driver_id)
    ...
    async def test_get_load_verification_status(self):
        ...
        with patch.object(self.service.supabase, 'get_load_by_id') as mock_get_load, \
             patch.object(self.service.supabase, 'check_load_ratecon_verified') as mock_check_ratecon:
    ...
    async def test_comprehensive_production_scenario(self):
        ...
        # Mock all services
        with patch.object(self.service.supabase, 'update_driver_flags') as mock_update_flags, \
             patch.object(self.service.supabase, 'update_load_status') as mock_update_status, \
             patch.object(self.service, '_update_load_ratecon_verified') as mock_update_ratecon, \
             patch.object(self.service.supabase, 'check_load_ratecon_verified') as mock_check_ratecon:
            ...
            cdl_result = await self.service.process_document_flags(cdl_doc, cdl_data, 0.95)
            coi_result = await self.service.process_document_flags(coi_doc, coi_data, 0.92)
            agreement_result = await self.service.process_document_flags(agreement_doc, agreement_data, 0.94)
            ratecon_result = await self.service.process_document_flags(ratecon_doc, ratecon_data, 0.88)
            pod_result = await self.service.process_document_flags(pod_doc, pod_data, 0.96)
            ...
            assert pod_result["invoice_ready"] is True  # Both POD and rate confirmation completed
