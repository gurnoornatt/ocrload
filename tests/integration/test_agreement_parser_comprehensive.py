...
from app.services.document_parsers.agreement_parser import (
    AgreementParser,
...
class TestAgreementParserComprehensive:
    ...
    def test_critical_requirement_agreement_signed_flag(self):
        ...
        ), f"Expected confidence >= 0.90, got {result.confidence:.3f}"
        assert (
            result.agreement_signed is True
        ), "CRITICAL: agreement_signed should be True when confidence >= 0.9"
        ...
        assert (
            result_low.confidence < 0.90
        ), f"Expected confidence < 0.90, got {result_low.confidence:.3f}"
        assert (
            result_low.agreement_signed is False
        ), "agreement_signed should be False when confidence < 0.9"

        print("\n✅ CRITICAL REQUIREMENT VALIDATED!")
        print(
            f"   - High confidence ({result.confidence:.3f}) → agreement_signed = {result.agreement_signed}"
        )
        print(
            f"   - Low confidence ({result_low.confidence:.3f}) → agreement_signed = {result_low.agreement_signed}"
        )
    ...
    def test_signature_detection_robustness(self):
        ...
        for test_case in signature_test_cases:
            ...
            print(
                f"  {test_case['name']:20} | Expected: {test_case['should_detect']:5} | Got: {detected:5} | {'✅' if detected == test_case['should_detect'] else '❌'}"
            )

            assert (
                detected == test_case["should_detect"]
            ), f"Signature detection failed for {test_case['name']}"
    ...
    def test_agreement_type_extraction_accuracy(self):
        ...
        for input_text, expected in type_test_cases:
            ...
            assert (
                result == expected
            ), f"Type extraction failed: expected '{expected}', got '{result}'"
    ...
    def test_confidence_scoring_logic(self):
        ...
        test_scenarios = [
            ...
            },
            {
                ...
                "data": AgreementData(
                    signature_detected=True, key_terms=["payment", "terms"]
                ),
                "expected_min": 0.70,
                "should_be_signed": False,
            },
            {
                "name": "Poor Agreement",
                "data": AgreementData(agreement_type="Some Agreement"),
                "expected_min": 0.0,
                "expected_max": 0.70,
                "should_be_signed": False,
            },
        ...
        for scenario in test_scenarios:
            ...
            print(
                f"  {scenario['name']:15} | Confidence: {confidence:.3f} | Signed: {is_signed:5} | {'✅' if is_signed == scenario['should_be_signed'] else '❌'}"
            )

            assert (
                confidence >= scenario["expected_min"]
            ), f"{scenario['name']} confidence too low"
            if "expected_max" in scenario:
                assert (
                    confidence <= scenario["expected_max"]
                ), f"{scenario['name']} confidence too high"
            assert (
                is_signed == scenario["should_be_signed"]
            ), f"{scenario['name']} signed flag incorrect"
    ...
    def test_ocr_integration_scenarios(self):
        ...
        for scenario in ocr_scenarios:
            ...
            status = (
                "✅"
                if result.data.signature_detected == scenario["should_detect_signature"]
                else "❌"
            )
            print(
                f"  {scenario['name']:15} | Signature: {result.data.signature_detected:5} | Confidence: {result.confidence:.3f} {status}"
            )

            assert (
                result.data.signature_detected == scenario["should_detect_signature"]
            ), f"OCR parsing failed for {scenario['name']}"
    ...
    def test_real_world_edge_cases(self):
        ...
        for case in edge_cases:
            try:
                ...
                print(
                    f"  {case['name']:20} | Confidence: {result.confidence:.3f} | Signed: {result.agreement_signed:5} {status}"
                )

                # Should always return a result, even for poor documents
                assert isinstance(
                    result, AgreementParsingResult
                ), f"Should return AgreementParsingResult for {case['name']}"
                assert (
                    0.0 <= result.confidence <= 1.0
                ), f"Confidence should be between 0 and 1 for {case['name']}"
    ...
    def test_performance_and_consistency(self):
        ...
        assert all(
            abs(r - results[0]) < 0.001 for r in results
        ), "Results should be consistent across runs"

        print("  Consistency Test: ✅ (all results within 0.001)")
    ...
    def test_final_validation(self):
        ...
        print(f"  📅 Signing Date: {result.data.signing_date}")
        print(
            f"  📝 Key Terms: {len(result.data.key_terms) if result.data.key_terms else 0} found"
        )
        ...
        assert (
            result.confidence >= 0.90
        ), f"Should have high confidence: {result.confidence:.3f}"
        assert (
            result.agreement_signed is True
        ), "CRITICAL: Should set agreement_signed = True for high confidence"
