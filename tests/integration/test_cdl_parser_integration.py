...
def create_cdl_image_with_text(text_content, width=600, height=400):
    ...
    for line in lines:
        ...
        current_font = header_font if any(title in line.upper() for title in ['COMMERCIAL', 'LICENSE', 'STATE']) else font
    ...
    try:
        ...
        async with UnifiedOCRClient() as ocr_client:
            ocr_result = await ocr_client.process_file_content(
                file_content=cdl_image,
                filename="test_cdl.png",
                mime_type="image/png"
            )
        
        print(f"✅ OCR completed with confidence: {ocr_result.get('average_confidence', 0):.2f}")
        
        print("📊 Parsing CDL from OCR result...")
        parsing_result = parser.parse_from_ocr_result(ocr_result)
        
        print(f"✅ CDL parsing completed!")
...
""",
    ...
    for test_case in test_cases:
        ...
        if min_conf <= confidence <= max_conf:
            ...
            print(f"❌ Confidence {confidence:.2f} not in expected range [{min_conf:.2f}, {max_conf:.2f}]")
            all_passed = False
...
async def main():
    ...
    if 'text_parsing' in test_results and isinstance(test_results['text_parsing'], dict):
    ...
    # OCR integration
    total_tests += 1
    if 'ocr_integration' in test_results and not test_results['ocr_integration'].get('error'):
        passed_tests += 1
        print(f"✅ ocr_integration: passed")
    else:
        print(f"❌ ocr_integration: failed")
