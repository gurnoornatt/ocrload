...
async def test_marker_vs_datalab_comparison():
    ...
    try:
        ...
        print("-" * 40)
        print(
            f"Marker - Pages: {marker_result.get('page_count')}, Confidence: {marker_result.get('average_confidence', 0):.2f}"
        )
        print(
            f"OCR    - Pages: {ocr_result.get('page_count')}, Confidence: {ocr_result.get('average_confidence', 0):.2f}"
        )
