...
try:
    ...
    from app.services.ocr_clients.unified_ocr_client import UnifiedOCRClient, UnifiedOCRError
...
async def test_normal_operation():
    ...
    test_content = create_test_image(text_lines=["INVOICE", "Customer: John Doe", "Amount: $123.45"])
    ...
    async with UnifiedOCRClient(confidence_threshold=0.5) as client:
        try:
            ...
            print(f"📊 Result: {result.get('extraction_method')} - confidence: {result.get('average_confidence', 0):.2f}")
            ...
            print(f"📈 Stats: {stats['datalab_success']} Datalab, {stats['marker_fallback']} Marker")
...
async def test_confidence_triggered_fallback():
    ...
    async with UnifiedOCRClient(confidence_threshold=0.99) as client:
        try:
            ...
            print("✅ Processing successful!")
            print(f"📊 Method: {result.get('extraction_method')} - confidence: {result.get('average_confidence', 0):.2f}")
...
async def test_forced_fallback():
    ...
    async with UnifiedOCRClient() as client:
        try:
            ...
            test_content = create_test_image(text_lines=["Document Title", "Content goes here"])
            result = await client.process_file_content(
                file_content=test_content,
...
async def test_statistics_tracking():
    ...
    async with UnifiedOCRClient(confidence_threshold=0.8) as client:
        try:
            ...
            for i in range(3):
                test_content = create_test_image(text_lines=[f"Document {i+1}", "Test content"])
                await client.process_file_content(
                    file_content=test_content,
...
async def test_fallback_disabled():
    ...
    async with UnifiedOCRClient(enable_fallback=False, confidence_threshold=0.99) as client:
        try:
