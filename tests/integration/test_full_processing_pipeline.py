...
class TestFullProcessingPipeline:
    ...
    def mock_database_flags(self):
        ...
        with patch.object(
            database_flag_service, "process_document_flags"
        ) as mock_process:
            mock_process.return_value = {
                "success": True,
                "flags_updated": {"ratecon_verified": True},
            }
            yield mock_process
    ...
    def test_retry_logic_on_temporary_failures(
        ...
        respx.post("https://api.datalab.to/api/v1/ocr").mock(
            side_effect=datalab_side_effect
        )
    ...
    def _wait_for_processing(
        self, client: TestClient, document_id: str, max_wait: int = 30
    ) -> dict[str, Any]:
        """
...
class TestErrorScenarios:
    ...
    def test_invalid_file_format_error_handling(self, client: TestClient):
        ...
        with tempfile.NamedTemporaryFile(suffix=".txt", delete=False) as temp_file:
            ...
            test_request = {"path": temp_file.name, "doc_type": "POD"}
...
def create_realistic_ocr_response(
    doc_type: str, confidence: float = 0.90
) -> dict[str, Any]:
