...
class TestRedisEventServiceIntegration:
    ...
    async def test_emit_custom_event_integration(self):
        ...
        result = await redis_event_service.emit_custom_event(
            ...
            payload={"test_data": "integration_test", "timestamp_test": datetime.now(timezone.utc).isoformat()}
        )
    ...
    async def test_database_flag_service_integration(self):
        ...
        with patch.object(database_flag_service.supabase, 'update_load_status') as mock_update_status, \
             patch.object(database_flag_service.supabase, 'check_load_ratecon_verified', return_value=True) as mock_check_ratecon, \
             patch.object(database_flag_service.supabase, 'get_load_by_id') as mock_get_load:
            ...
            result = await database_flag_service.process_document_flags(
                document=document,
                parsed_data=pod_data.model_dump(),
                confidence=0.95
            )
            ...
            if health["status"] == "healthy":
                ...
                assert any("Invoice ready event emitted to Redis" in rule for rule in business_rules)
            else:
    ...
    async def test_database_flag_service_no_event_when_not_ready(self):
        ...
        with patch.object(database_flag_service.supabase, 'update_load_status') as mock_update_status, \
             patch.object(database_flag_service.supabase, 'check_load_ratecon_verified', return_value=False) as mock_check_ratecon:
            ...
            business_rules = result["business_rules_applied"]
            assert any("Invoice not ready: rate confirmation not verified" in rule for rule in business_rules)
    ...
    async def test_redis_connection_retry_logic(self):
        ...
        try:
            ...
            redis_event_service._last_connection_attempt = datetime.now(timezone.utc)
            
            # Try to emit event (should fail due to recent failure)
            result1 = await redis_event_service.emit_invoice_ready_event(uuid4(), uuid4())
            assert result1 is False
            
            # Simulate time passing (set last attempt to past)
            past_time = datetime.now(timezone.utc).timestamp() - 400  # 6+ minutes ago
            redis_event_service._last_connection_attempt = datetime.fromtimestamp(past_time, timezone.utc)
            
            # Try again (should attempt retry)
            result2 = await redis_event_service.emit_invoice_ready_event(uuid4(), uuid4())
