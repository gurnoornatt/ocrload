...
class TestAgreementParserIntegration:
    ...
    def test_driver_agreement_text_parsing(self):
        ...
        for agreement_type, agreement_text in agreement_samples.items():
            ...
            if agreement_type in [
                ...
                ), f"{agreement_type} should detect signature"
                assert (
                    result.data.agreement_type is not None
                ), f"{agreement_type} should extract agreement type"
                assert (
                    result.confidence >= 0.70
                ), f"{agreement_type} should have decent confidence: {result.confidence}"
                ...
                assert (
                    result.data.agreement_type == "Terms and Conditions"
                ), f"{agreement_type} should extract type"
                assert (
                    result.confidence >= 0.70
                ), f"{agreement_type} should have decent confidence: {result.confidence}"
                print(f"  ✅ {agreement_type} agreement parsing successful")
                ...
                assert (
                    result.data.signature_detected is True
                ), f"{agreement_type} should detect electronic agreement"
                assert (
                    result.data.agreement_type == "Transportation Agreement"
                ), f"{agreement_type} should extract type"
                print(f"  ✅ {agreement_type} agreement parsing successful")
    ...
    def test_agreement_confidence_thresholds(self):
        ...
        for case in test_cases:
            ...
            if case["expected_signed"]:
                ...
                ), f"Should have confidence >= {case['expected_confidence']}"
                assert result.agreement_signed is True, "Should be marked as signed"
            else:
                assert (
                    result.agreement_signed is False
    ...
    async def test_agreement_with_ocr_integration(self):
        ...
        for agreement_type, mock_data in agreement_ocr_samples.items():
            ...
            ), f"{agreement_type} should have reasonable confidence"
    ...
    def test_agreement_edge_cases(self):
        ...
        for case_name, case_data in edge_cases.items():
            ...
            assert (
                result.data.signature_detected == case_data["should_detect_signature"]
            ), f"{case_name} signature detection mismatch"
            assert (
                result.confidence >= case_data["min_confidence"]
    ...
    def test_full_integration_workflow(self):
        ...
        if result.confidence >= 0.90:
            ...
            ), "Task requirement: agreement_signed = True when confidence >= 0.9"
