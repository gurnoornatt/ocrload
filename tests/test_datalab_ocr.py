...
class TestDatalabOCRClient:
    ...
    async def test_poll_for_results_processing_then_complete(self):
        ...
        processing_response = {"status": "processing", "success": True}
        ...
        with patch(
            ...
            result = await client._poll_for_results(
                check_url="https://test.com/check", max_polls=10, poll_interval=0.01
            )
    ...
    async def test_process_file_content_success(self):
        ...
        with patch.object(
            client, "_submit_ocr_request", return_value=submit_response
        ) as mock_submit:
            with patch.object(
                client, "_poll_for_results", return_value=poll_response
            ) as mock_poll:
        ...
        with patch("aiofiles.open") as mock_aio_open:
            ...
            with patch.object(Path, "exists", return_value=True):
                with patch("mimetypes.guess_type", return_value=("image/jpeg", None)):
                    with patch.object(
                        client, "process_file_content", return_value=expected_result
                    ) as mock_process:
                        result = await client.process_file_path(
                            file_path="test.jpg", languages=["English"]
                        )
            ...
            with patch.object(Path, "exists", return_value=True):
                with patch(
                    ...
                    with patch.object(
                        ...
                        await client.process_file_path("document.pdf")
