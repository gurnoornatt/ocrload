...
async def migrate_documents_table():
    ...
    for i, migration in enumerate(migrations, 1):
        try:
            ...
            result = client.rpc('exec_sql', {'sql': migration}).execute()
            ...
            try:
                ...
                result = client.postgrest.rpc('exec_sql', {'sql': migration}).execute()
                logger.info(f"Migration {i} completed with alternative approach")
            except Exception as e2:
                logger.error(f"Alternative approach also failed for migration {i}: {e2}")
                logger.warning("You may need to run these migrations manually in the Supabase dashboard:")
    ...
    try:
        ...
        result = client.table('documents').select('id, driver_id, load_id, type, url, status, updated_at, metadata, parsed_data, confidence, verified, created_at').limit(1).execute()
        logger.info("✓ All columns are now accessible")
        
        # Show the current schema
        columns_to_test = ['id', 'driver_id', 'load_id', 'type', 'url', 'status', 'updated_at', 'metadata', 'parsed_data', 'confidence', 'verified', 'created_at']
        existing_columns = []
        
        for col in columns_to_test:
            try:
                result = client.table('documents').select(col).limit(1).execute()
