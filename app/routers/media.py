...
from app.models.responses import MediaUploadResponse, ProcessingStatusResponse, DocumentFlags
from app.config.settings import settings
from app.exceptions import (
    DocumentNotFoundError, DownloadError, StorageError, DatabaseError,
    OCRError, DocumentParsingError, NetworkError, to_http_exception
)
...
async def process_document_pipeline(
    ...
    try:
        logger.info(f"Starting document processing pipeline for document {document_id} - Request ID: {request_id}")
        ...
        try:
            ...
            file_content, filename, content_type_local = await document_storage_service.download_file_from_url(media_url)
        ...
        logger.info(f"Document processing pipeline completed successfully for {document_id} in {processing_time_ms}ms")
        
    except Exception as e:
        logger.error(f"Unexpected error in document processing pipeline for {document_id}: {e}")
        try:
            await document_service.update_document_status(
                document_id=document_id,
                status=DocumentStatus.FAILED,
...
async def upload_media(
    ...
    try:
        ...
        logger.error(f"Unexpected error in media upload: {e} - Request ID: {request_id}")
        raise HTTPException(
            status_code=500,
...
async def get_processing_status(
    ...
    try:
        ...
        logger.error(f"Error getting processing status for {document_id}: {e} - Request ID: {request_id}")
        raise HTTPException(
            status_code=500,
