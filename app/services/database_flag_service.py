...
from app.models.database import (
    Document, DocumentType, LoadStatus, 
...
class DatabaseFlagUpdateService:
    ...
    async def process_document_flags(
        ...
        try:
            ...
            if document.type == DocumentType.CDL:
                ...
                await self._process_agreement_flags(document, parsed_data, confidence, result)
            elif document.type == DocumentType.RATE_CON:
                await self._process_ratecon_flags(document, parsed_data, confidence, result)
    ...
    async def _process_cdl_flags(
        ...
        if confidence >= self.MIN_CONFIDENCE_THRESHOLD:
            result["business_rules_applied"].append(f"Confidence threshold met: {confidence:.2f} >= {self.MIN_CONFIDENCE_THRESHOLD}")
            
            # Check expiry date requirement
            if cdl_data.expiration_date:
                min_expiry_date = datetime.now(timezone.utc) + timedelta(days=self.CDL_MIN_DAYS_BEFORE_EXPIRY)
                ...
                if cdl_data.expiration_date > min_expiry_date:
                    ...
                    logger.warning(f"CDL expiry too soon for driver {document.driver_id}")
            else:
                result["business_rules_applied"].append("CDL expiry date not found - cannot verify")
                logger.warning(f"CDL missing expiry date for driver {document.driver_id}")
        else:
            result["business_rules_applied"].append(f"Confidence too low: {confidence:.2f} < {self.MIN_CONFIDENCE_THRESHOLD}")
    ...
    async def _process_coi_flags(
        ...
        if confidence >= self.MIN_CONFIDENCE_THRESHOLD:
            ...
            if coi_data.expiration_date:
                ...
                result["business_rules_applied"].append("COI expiry date not found - cannot verify")
                logger.warning(f"COI missing expiry date for driver {document.driver_id}")
        else:
            result["business_rules_applied"].append(f"Confidence too low: {confidence:.2f} < {self.MIN_CONFIDENCE_THRESHOLD}")
    ...
    async def _process_agreement_flags(
        ...
        if confidence >= self.MIN_CONFIDENCE_THRESHOLD:
            ...
            if agreement_data.signature_detected:
                ...
                result["business_rules_applied"].append("Signature detected in agreement")
                ...
                result["flags_updated"]["agreement_signed"] = True
                result["business_rules_applied"].append("Agreement signed based on high confidence (no signature detection)")
                logger.info(f"Agreement verified for driver {document.driver_id} (no signature detected)")
        else:
            result["business_rules_applied"].append(f"Confidence too low: {confidence:.2f} < {self.MIN_CONFIDENCE_THRESHOLD}")
    ...
    async def _process_ratecon_flags(
        ...
        has_origin = ratecon_data.origin is not None and len(ratecon_data.origin.strip()) > 0
        has_destination = ratecon_data.destination is not None and len(ratecon_data.destination.strip()) > 0
        ...
        result["business_rules_applied"].append(f"Required fields present: {required_fields}")
        ...
        if has_rate and has_origin and has_destination:
            ...
            result["business_rules_applied"].append("All required fields present: rate, origin, destination")
            ...
            result["business_rules_applied"].append(f"Missing required fields: {missing_fields}")
            logger.warning(f"Rate confirmation incomplete for load {document.load_id}: missing {missing_fields}")
    ...
    async def _process_pod_flags(
        ...
        if confidence >= self.MIN_CONFIDENCE_THRESHOLD:
            ...
            if pod_data.delivery_confirmed:
                ...
                result["business_rules_applied"].append("Delivery confirmed - load marked as delivered")
                ...
                logger.warning(f"POD for load {document.load_id} has high confidence but delivery not confirmed")
        else:
            result["business_rules_applied"].append(f"Confidence too low: {confidence:.2f} < {self.MIN_CONFIDENCE_THRESHOLD}")
    
    async def _update_load_ratecon_verified(self, load_id: Union[str, UUID], verified: bool) -> None:
        """
        ...
        try:
            ...
            result = self.supabase.client.table("documents").update({
                "verified": verified
            }).eq("load_id", str(load_id)).eq("type", "RATE_CON").execute()
            
            logger.info(f"Updated rate confirmation verification for load {load_id}: {verified}")
            
        except Exception as e:
            logger.error(f"Failed to update rate confirmation status for load {load_id}: {e}")
            raise
    
    async def _check_invoice_readiness(self, load_id: Union[str, UUID], result: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            if ratecon_verified:
                result["business_rules_applied"].append("Invoice ready: POD completed + rate confirmation verified")
                ...
                if load and load.assigned_driver_id:
                    ...
                    result["business_rules_applied"].append("Invoice ready event emitted to Redis")
                else:
                    logger.warning(f"Cannot emit invoice_ready event for load {load_id}: assigned_driver_id not found")
                    result["business_rules_applied"].append("Invoice ready event not emitted: assigned_driver_id missing")
                
            else:
                result["business_rules_applied"].append("Invoice not ready: rate confirmation not verified")
    ...
    async def get_driver_verification_status(self, driver_id: Union[str, UUID]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "verification_complete": (
                    driver.doc_flags.cdl_verified and 
                    driver.doc_flags.insurance_verified and 
                    driver.doc_flags.agreement_signed
                ),
                "updated_at": driver.updated_at.isoformat()
            }
            
        except Exception as e:
            logger.error(f"Failed to get driver verification status for {driver_id}: {e}")
            raise
    
    async def get_load_verification_status(self, load_id: Union[str, UUID]) -> Dict[str, Any]:
        """
