...
class PODParser:
    ...
    MEDIUM_CONFIDENCE_THRESHOLD = 0.70  # Delivery confirmed + (signature OR receiver OR date)
    ...
    def _compile_regex_patterns(self):
        ...
        self.receiver_patterns = [
            ...
            re.compile(r'(?:received|delivered|signed)\s+(?:to|by)[:\s]*(?:mr\.?|ms\.?|mrs\.?|dr\.?)?\s*([A-Za-z][A-Za-z\s]{2,30})', re.IGNORECASE),
            re.compile(r'consignee[:\s]*(?:mr\.?|ms\.?|mrs\.?)?\s*([A-Za-z][A-Za-z\s]{2,30})', re.IGNORECASE),
            re.compile(r'recipient[:\s]*(?:mr\.?|ms\.?|mrs\.?)?\s*([A-Za-z][A-Za-z\s]{2,30})', re.IGNORECASE),
            re.compile(r'customer[:\s]*(?:mr\.?|ms\.?|mrs\.?)?\s*([A-Za-z][A-Za-z\s]{2,30})', re.IGNORECASE),
            # Generic patterns
            ...
            re.compile(r'signature[:\s]*[_\-]*\s*([A-Za-z][A-Za-z\s]{2,30})', re.IGNORECASE),
        ]
        
        # Delivery date/time patterns  
        self.delivery_date_patterns = [
            # With delivery keywords
            re.compile(r'(?:delivered|delivery|received)\s+(?:on|at)?[:\s]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE),
            re.compile(r'(?:delivered|delivery|received)\s+(?:on|at)?[:\s]*(\d{4}[/-]\d{1,2}[/-]\d{1,2})', re.IGNORECASE),
            re.compile(r'delivery\s+date[:\s]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE),
            re.compile(r'delivery\s+date[:\s]*(\d{4}[/-]\d{1,2}[/-]\d{1,2})', re.IGNORECASE),
            re.compile(r'delivered[:\s]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE),
            re.compile(r'delivered[:\s]*(\d{4}[/-]\d{1,2}[/-]\d{1,2})', re.IGNORECASE),
            # Time inclusion
            re.compile(r'(?:delivered|delivery|received)[:\s]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})\s+(?:at\s+)?(\d{1,2}:\d{2})', re.IGNORECASE),
            re.compile(r'(?:delivered|delivery|received)[:\s]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})\s+(\d{1,2}:\d{2}\s*[ap]m)', re.IGNORECASE),
        ...
        self.notes_patterns = [
            ...
            re.compile(r'(?:delivery\s+)?notes?\b[:\s]*([^\n\r]{10,200})', re.IGNORECASE),
            re.compile(r'(?:special\s+)?instructions?\b[:\s]*([^\n\r]{10,200})', re.IGNORECASE),
    ...
    def parse(self, text_content: str) -> PODParsingResult:
        ...
        delivery_confirmed = self._extract_delivery_confirmation(cleaned_text, extraction_details)
        signature_present = self._extract_signature_presence(cleaned_text, extraction_details)
        ...
        logger.info(f"POD parsing completed: confidence={confidence:.2f}, pod_completed={pod_completed}")
    ...
    def _extract_delivery_confirmation(self, text: str, details: Dict[str, Any]) -> bool:
        ...
        for indicator in self.pod_indicators:
            if indicator in text_lower:
                ...
                logger.debug(f"Delivery confirmation inferred from document type: {indicator}")
                return True
    ...
    def _extract_signature_presence(self, text: str, details: Dict[str, Any]) -> bool:
        ...
        text_lower = text.lower()
        signature_keywords = [
            'signature', 'signed', 'electronic signature', 'digital signature',
            'signature on file', 'signed by', 'received by', 'accepted by'
        ]
        ...
        if signature_indicators:
            ...
            details['signature_indicators'] = signature_indicators[:5]  # Limit to avoid clutter
            logger.debug(f"Signature presence detected: {len(signature_indicators)} indicators")
            return True
    ...
    def _extract_receiver_name(self, text: str, details: Dict[str, Any]) -> Optional[str]:
        ...
        for pattern in self.receiver_patterns:
            ...
            if matches:
                ...
                for match in matches:
                    ...
                    bad_words = [
                        'date', 'time', 'signature', 'line', 'print', 'page',
                        'delivery', 'package', 'condition', 'satisfied',
                        'front door', 'good', 'excellent', 'poor', 'damaged',
                    ...
                    if '\n' in name or '\r' in name:
                        ...
                        if len(clean_name) >= 2 and not any(word in clean_name.lower() for word in bad_words):
                            name = clean_name
                        else:
                            continue
                    ...
                    if re.match(r'^[A-Z][a-z]+ [A-Z][a-z]+$', name):  # "John Smith" pattern
                        score += 5
    ...
    def _extract_delivery_date(self, text: str, details: Dict[str, Any]) -> Optional[datetime]:
    ...
    def _extract_delivery_notes(self, text: str, details: Dict[str, Any]) -> Optional[str]:
        ...
        if notes_found:
            ...
            combined_notes = '. '.join(dict.fromkeys(notes_found))  # Remove duplicates while preserving order
    ...
    def _calculate_confidence(self, pod_data: PODData, details: Dict[str, Any]) -> float:
    ...
    def _clean_ocr_artifacts(self, text: str) -> str:
        ...
        for error, correction in ocr_fixes.items():
            ...
            cleaned_text = re.sub(re.escape(error), correction, cleaned_text, flags=re.IGNORECASE)
