...
class CDLParser:
    ...
    def _compile_regex_patterns(self):
        ...
        self.name_patterns = [
            ...
            re.compile(
                r"(?:NAME|Name):\s*([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+){1,3})",
                re.IGNORECASE,
            ),
            # "SMITH, JOHN" format
            re.compile(
                r"([A-Z][A-Z]+,\s*[A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)*)", re.IGNORECASE
            ),
            # "First: John Last: Smith" format
            re.compile(
                r"(?:First|FIRST):\s*([A-Z][a-zA-Z]+).*?(?:Last|LAST):\s*([A-Z][a-zA-Z]+)",
                re.IGNORECASE | re.DOTALL,
            ),
            # "John Smith" on its own line (common in many states) - more restrictive
            re.compile(
                r"^([A-Z][a-z]+\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)$", re.MULTILINE
            ),
        ]
        ...
        self.license_patterns = [
            ...
            re.compile(
                r"(?:CA|TX|FL|NY|IL|PA|OH|GA|NC|MI)\s*([A-Z0-9]{7,12})", re.IGNORECASE
            ),
        ]
        ...
        self.expiration_patterns = [
            ...
            re.compile(
                r"(?:EXP|EXPIRES|EXPIRATION)\s*(?:DATE)?[:]*\s*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})",
                re.IGNORECASE,
            ),
            # "DOB: MM/DD/YYYY EXP: MM/DD/YYYY" - capture the second date
            re.compile(
                r"DOB:\s*\d{1,2}[/-]\d{1,2}[/-]\d{2,4}.*?(?:EXP|EXPIRES)[:]*\s*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})",
                re.IGNORECASE,
            ),
        ...
        self.address_patterns = [
            ...
            re.compile(
                r"(?:ADDRESS|ADDR)[:]*\s*([0-9]+\s+[A-Za-z\s]+(?:ST|STREET|AVE|AVENUE|RD|ROAD|BLVD|BOULEVARD|DR|DRIVE|LN|LANE)[^0-9]*?[A-Z]{2}\s+\d{5})",
                re.IGNORECASE,
            ),
            # Multi-line address format - more specific
            re.compile(
                r"([0-9]+\s+[A-Za-z][A-Za-z\s]+(?:ST|STREET|AVE|AVENUE|RD|ROAD|BLVD|BOULEVARD|DR|DRIVE|LN|LANE))[^0-9]*?([A-Z]{2}\s+\d{5})",
                re.IGNORECASE | re.DOTALL,
            ),
            # Simple format: "123 Main St" - but exclude license numbers
            re.compile(
                r"([0-9]{1,5}\s+[A-Za-z][A-Za-z\s]+(?:ST|STREET|AVE|AVENUE|RD|ROAD|BLVD|BOULEVARD|DR|DRIVE|LN|LANE))(?![0-9])",
                re.IGNORECASE,
            ),
        ]
    ...
    def _extract_name(self, text: str, details: dict[str, Any]) -> str | None:
        ...
        for i, pattern in enumerate(self.class_patterns):
            ...
            if match:
                ...
                if license_class in ["A", "B", "C"]:
                    ...
                    logger.debug(
                        f"Extracted license class: {license_class} using pattern {i}"
                    )
    ...
    def _extract_address(self, text: str, details: dict[str, Any]) -> str | None:
    ...
    def _extract_state(self, text: str, details: dict[str, Any]) -> str | None:
    ...
    def _clean_name(self, name: str) -> str:
        ...
        for word in name_words:
            ...
            if (
                re.match(r"^[A-Z0-9]{7,}$", word)  # License number pattern
                or re.match(r"\d+[/-]\d+", word)  # Date pattern
                or word.upper() in ["LICENSE", "CDL", "EXP", "EXPIRES", "CLASS"]
            ):
                continue
    ...
    def _calculate_confidence(
        self, cdl_data: CDLData, details: dict[str, Any]
    ) -> float:
        """
        ...
        has_name_and_expiry = (
            extracted_fields["name"] and extracted_fields["expiration"]
        )
    ...
    def _is_cdl_verified(self, cdl_data: CDLData) -> bool:
        ...
        if days_until_expiry < self.MIN_EXPIRATION_DAYS:
            logger.warning(
                f"CDL expires in {days_until_expiry} days, less than required {self.MIN_EXPIRATION_DAYS}"
            )
            return False
