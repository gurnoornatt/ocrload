...
class RateConfirmationParser:
    ...
    def _compile_regex_patterns(self):
        ...
        self.rate_patterns = [
            ...
            re.compile(r'(?:rate|amount|total)[:\s]*\$?([0-9]{1,5}(?:,[0-9]{3})*(?:\.[0-9]{2})?)', re.IGNORECASE),
            re.compile(r'compensation[:\s]*\$?([0-9]{1,5}(?:,[0-9]{3})*(?:\.[0-9]{2})?)', re.IGNORECASE),
            re.compile(r'pay[:\s]*\$?([0-9]{1,5}(?:,[0-9]{3})*(?:\.[0-9]{2})?)', re.IGNORECASE),
            ...
            re.compile(r'([0-9]{1,5}(?:,[0-9]{3})*(?:\.[0-9]{2})?)\s*(?:dollars?|usd)', re.IGNORECASE),
        ]
        
        # Origin/destination patterns
        self.location_patterns = [
            # "FROM: City, ST" or "ORIGIN: City, ST"
            re.compile(r'(?:from|origin|pickup)[:\s]*([A-Za-z\s]+),\s*([A-Z]{2})', re.IGNORECASE),
            # "TO: City, ST" or "DESTINATION: City, ST"
            re.compile(r'(?:to|destination|delivery)[:\s]*([A-Za-z\s]+),\s*([A-Z]{2})', re.IGNORECASE),
        ...
        self.date_patterns = [
            ...
            re.compile(r'(?:pickup|pick.?up|loading)[:\s]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE),
            re.compile(r'(?:delivery|deliver|unload)[:\s]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE),
        ...
        self.commodity_patterns = [
            re.compile(r'(?:commodity|product|freight|cargo)[:\s]*([A-Za-z\s,.-]+)', re.IGNORECASE),
    ...
    def parse(self, text_content: str) -> RateConfirmationParsingResult:
        ...
        Returns:
            RateConfirmationParsingResult with extracted data
        """
        logger.info(f"Parsing rate confirmation from {len(text_content)} characters of text")
        ...
        pickup_date, delivery_date = self._extract_dates(cleaned_text, extraction_details)
    ...
    def _extract_rate_amount(self, text: str, details: Dict[str, Any]) -> Optional[int]:
    ...
    def _extract_locations(self, text: str, details: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
        ...
        destination_keywords = ['to', 'destination', 'delivery', 'deliver', 'drop off', 'drop', 'unload']
        ...
        for line in lines:
            ...
            for keyword in origin_keywords:
                ...
                if match and not origin:
                    ...
                    remaining_line = remaining_line.lstrip(':').strip()  # Remove colon if present
                    
                    # Try multiple location patterns
                    location_patterns = [
                        # City, ST format
                        re.compile(r'([A-Za-z][A-Za-z\s]*[A-Za-z]),\s*([A-Z]{2})', re.IGNORECASE),
                    ...
                    for loc_pattern in location_patterns:
                        ...
                        if location_match:
                            if len(location_match.groups()) >= 2:
                                ...
                                # Just city name, try to infer state or use as-is
                                city = location_match.group(1).strip()
                                destination = f"{city}, XX"  # Placeholder for missing state
                            details['destination_extraction'] = f'Found destination: {destination}'
                            break
        ...
        if not origin or not destination:
            ...
            from_to_patterns = [
                ...
                re.compile(r'from\s+([A-Za-z]+(?:\s+[A-Za-z]+)*),\s*([A-Z]{2})\s+to\s+([A-Za-z]+(?:\s+[A-Za-z]+)*),\s*([A-Z]{2})', re.IGNORECASE),
                # Abbreviated format: "Chicago IL to Detroit MI" or "Houston TX to Dallas TX"
                re.compile(r'([A-Za-z]+(?:\s+[A-Za-z]+)*)\s+([A-Z]{2})\s+to\s+([A-Za-z]+(?:\s+[A-Za-z]+)*)\s+([A-Z]{2})', re.IGNORECASE),
                # Even more flexible: "pick up Houston TX drop off Dallas TX"
                re.compile(r'(?:pick\s*up|pickup)\s+([A-Za-z]+(?:\s+[A-Za-z]+)*)\s+([A-Z]{2}).*?(?:drop\s*off|delivery?)\s+([A-Za-z]+(?:\s+[A-Za-z]+)*)\s+([A-Z]{2})', re.IGNORECASE),
            ]
            ...
            for pattern in from_to_patterns:
                ...
                if match:
                    if not origin:
                        ...
                        details['origin_extraction'] = f'Found origin: {origin}'
                    if not destination:
                        destination = f"{match.group(3).strip()}, {match.group(4).upper()}"
                        details['destination_extraction'] = f'Found destination: {destination}'
                    break
            ...
            if not (origin and destination):
                ...
                if len(all_locations) > 1 and not destination:
                    ...
                    details['destination_extraction'] = f'Inferred destination: {destination}'
    ...
    def _extract_dates(self, text: str, details: Dict[str, Any]) -> Tuple[Optional[datetime], Optional[datetime]]:
            ...
            for keyword in pickup_keywords:
                if keyword in line.lower():
                    ...
                    if date_match and not pickup_date:
                        ...
                        if pickup_date:
                            details['pickup_date_extraction'] = f'Found pickup date: {pickup_date.strftime("%m/%d/%Y")}'
                        break
            ...
            for keyword in delivery_keywords:
                if keyword in line.lower():
                    ...
                    if date_match and not delivery_date:
                        ...
                        if delivery_date:
                            details['delivery_date_extraction'] = f'Found delivery date: {delivery_date.strftime("%m/%d/%Y")}'
                        break
        ...
        if not pickup_date or not delivery_date:
            ...
            if all_dates and not pickup_date:
                ...
                details['pickup_date_extraction'] = f'Inferred pickup date: {pickup_date.strftime("%m/%d/%Y")}'
            if len(all_dates) > 1 and not delivery_date:
                delivery_date = all_dates[1]
                details['delivery_date_extraction'] = f'Inferred delivery date: {delivery_date.strftime("%m/%d/%Y")}'
    ...
    def _extract_weight(self, text: str, details: Dict[str, Any]) -> Optional[float]:
        """Extract load weight."""
            ...
            for match in matches:
                try:
                    ...
                    if 100 <= weight_value <= 80000:  # Reasonable weight range
                        details['weight_extraction'] = f'Found weight: {weight_value:,.0f} lbs'
                        return weight_value
                except (ValueError, TypeError):
                    continue
    ...
    def _extract_commodity(self, text: str, details: Dict[str, Any]) -> Optional[str]:
        ...
        for pattern in self.commodity_patterns:
            ...
            for match in matches:
                ...
                if (len(commodity) >= 3 and len(commodity) <= 100 and 
                    not commodity.lower() in ['here', 'there', 'this', 'that'] and
                    re.search(r'[A-Za-z]', commodity)):  # Must contain letters
    ...
    def _calculate_confidence(self, ratecon_data: RateConData, details: Dict[str, Any]) -> float:
        """
    ...
    def _is_ratecon_verified(self, ratecon_data: RateConData, confidence: float) -> bool:
        """
        ...
        """
        has_required_fields = (
            ratecon_data.rate_amount is not None and
            ratecon_data.origin is not None and
    ...
    def parse_from_ocr_result(self, ocr_result: Dict[str, Any]) -> RateConfirmationParsingResult:
        """
