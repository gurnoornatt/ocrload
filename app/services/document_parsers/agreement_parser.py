...
class AgreementParser:
    ...
    def _compile_regex_patterns(self):
        ...
        self.signature_patterns = [
            ...
            re.compile(r'(?:Digitally|D[0-9]g[0-9]tally|Electronic(?:ally)?)\s+(?:Signed|S[0-9]gn[e3]d)\s+(?:by|BY)[:]*\s*([A-Za-z0-9\s\.]+)', re.IGNORECASE),
            # Driver/Signature lines with OCR error tolerance - must have colon or specific signature context
            re.compile(r'(?:Driver|Dr[0-9]v[e3]r)\s+(?:Signature|S[0-9]gnatur[e3])[:](?:\s*([A-Za-z0-9_\s\.]*)|$)', re.IGNORECASE),
            re.compile(r'(?:Signature|S[0-9]gnatur[e3])[:](?:\s*([A-Za-z0-9_\s\.]*)|$)', re.IGNORECASE),
            re.compile(r'(?:Signed|S[0-9]gn[e3]d)\s+(?:by|BY)[:]*\s*([A-Za-z0-9_\s\.]+)', re.IGNORECASE),
            ...
            re.compile(r'(?:Date|Dat[e3])\s+(?:Signed|S[0-9]gn[e3]d)[:]*\s*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE),
            re.compile(r'(?:Signed|S[0-9]gn[e3]d)\s+(?:on|ON)[:]*\s*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE),
            # Electronic signature confirmation with OCR tolerance
            re.compile(r'(?:I\s+agree|I\s+accept|I\s+acknowledge).*(?:terms|agreement|contract|conditions|responsibility)', re.IGNORECASE),
        ]
        
        # Agreement type patterns
        self.agreement_type_patterns = [
            # Common agreement types with OCR error tolerance
            re.compile(r'(?:Driver|Dr[0-9]v[e3]r|Independent\s+Contractor|Carrier)\s+(?:Agreement|Agr[e3][e3]m[e3]nt)', re.IGNORECASE),
            ...
            re.compile(r'(?:^|\n)\s*Terms\s+(?:and\s+Conditions|of\s+Service)', re.IGNORECASE | re.MULTILINE),
        ...
        self.key_terms_patterns = [
            ...
            re.compile(r'(?:liability|insurance|coverage).*(?:amount|limit)[:]*\s*\$?([0-9,]+(?:\.[0-9]{2})?)', re.IGNORECASE),
            # Payment terms
            re.compile(r'(?:payment|compensation|rate).*(?:per|@).*(?:mile|load|hour)', re.IGNORECASE),
            # Equipment requirements
            re.compile(r'(?:equipment|vehicle|truck).*(?:requirement|specification)', re.IGNORECASE),
            # Termination clauses
            re.compile(r'(?:termination|cancel|terminate).*(?:notice|days|immediately)', re.IGNORECASE),
            # Compliance requirements
            re.compile(r'(?:compliance|regulation|DOT|FMCSA).*(?:requirement|standard)', re.IGNORECASE),
        ]
        
        # Signing date patterns
        self.signing_date_patterns = [
            # "Date Signed: 01/01/2025" or "Signed on: 01/01/2025"
            re.compile(r'(?:Date\s+Signed|Signed\s+on|Signature\s+Date)[:]*\s*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE),
            # "Date: 01/01/2025" near signature context
            re.compile(r'(?:Date)[:]*\s*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE),
            # "Agreed on: 01/01/2025"
            re.compile(r'(?:Agreed\s+on|Agreement\s+Date)[:]*\s*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE),
        ]
    ...
    def parse(self, ocr_text: str) -> AgreementParsingResult:
        ...
        """
        logger.info(f"Parsing Agreement document from {len(ocr_text)} characters of OCR text")
    ...
    def _detect_signature(self, text: str, details: Dict[str, Any]) -> bool:
        ...
        signature_detected = signature_indicators >= 2 or self._has_strong_signature_indicator(signature_details)
    ...
    def _get_signature_pattern_type(self, pattern_index: int) -> str:
        ...
        return pattern_types[pattern_index] if pattern_index < len(pattern_types) else 'unknown'
    ...
    def _has_strong_signature_indicator(self, signature_details: List[Dict]) -> bool:
        ...
        for detail in signature_details:
            ...
            if pattern_type in ['digital_signature', 'electronic_agreement', 'signature_marks', 'signed_date', 'signed_on']:
                return True
    ...
    def _extract_signing_date(self, text: str, details: Dict[str, Any]) -> Optional[datetime]:
        ...
        for i, pattern in enumerate(self.signing_date_patterns):
            ...
            if match:
                ...
                if parsed_date:
                    ...
                    logger.debug(f"Extracted signing date: {parsed_date} using pattern {i}")
                    return parsed_date
        
        details['signing_date_pattern'] = None
        return None
    
    def _extract_agreement_type(self, text: str, details: Dict[str, Any]) -> Optional[str]:
        ...
        for i, pattern in enumerate(self.agreement_type_patterns):
            ...
            if match:
                ...
                for j, word in enumerate(words):
                    ...
                    if j > 0 and word in ['and', 'of', 'the', 'in', 'on', 'at', 'to', 'for', 'with']:
                ...
                logger.debug(f"Extracted agreement type: {agreement_type} using pattern {i}")
                return agreement_type
        
        details['agreement_type_pattern'] = None
        return None
    
    def _extract_key_terms(self, text: str, details: Dict[str, Any]) -> Optional[List[str]]:
        """Extract key terms and clauses from agreement text."""
        key_terms = []
        
        for i, pattern in enumerate(self.key_terms_patterns):
            matches = pattern.findall(text)
            if matches:
    ...
    def _calculate_confidence(self, agreement_data: AgreementData, details: Dict[str, Any]) -> float:
        """
        ...
        has_terms = agreement_data.key_terms is not None and len(agreement_data.key_terms) > 0
    ...
    def parse_from_ocr_result(self, ocr_result: Dict[str, Any]) -> AgreementParsingResult:
        """
