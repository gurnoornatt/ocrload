...
class DocumentStorageService:
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        ...
        if self.session is None or self.session.closed:
            ...
            self.session = aiohttp.ClientSession(
                timeout=timeout, headers={"User-Agent": "OCR-Load-Service/1.0"}
            )
        return self.session
    ...
    def _generate_storage_path(
        self, driver_id: UUID, doc_type: str, original_filename: str
    ) -> str:
        """
    ...
    async def download_file_from_url(self, url: str) -> tuple[bytes, str, str]:
        ...
        try:
            ...
            async with session.get(url) as response:
                ...
                logger.info(
                    f"Successfully downloaded file: {filename} ({len(content)} bytes, {content_type})"
                )
        ...
        try:
            ...
            if not content_type or not self._validate_mime_type(content_type):
                ...
                if ext in [".jpg", ".jpeg"]:
                    ...
                    raise FileValidationError(
                        f"Cannot determine content type for {path.name}"
                    )
            ...
            logger.info(
                f"Successfully read local file: {path.name} ({len(content)} bytes, {content_type})"
            )
    ...
    async def upload_to_storage(
        ...
        try:
            ...
            storage_path = self._generate_storage_path(
                driver_id, doc_type, original_filename
            )
    ...
    async def process_url_upload(
        self, media_url: str, driver_id: UUID, doc_type: str
    ) -> dict[str, any]:
        """
        ...
        """
        # Download file
        file_content, filename, content_type = await self.download_file_from_url(
            media_url
        )
    ...
    async def process_local_upload(
        self, file_path: str, driver_id: UUID, doc_type: str
    ) -> dict[str, any]:
        """
