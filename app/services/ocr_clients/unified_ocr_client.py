...
class UnifiedOCRClient:
    ...
    def __init__(
        ...
        self.marker_client = (
            MarkerOCRClient(api_key=api_key) if enable_fallback else None
        )
    ...
    async def process_file_content(
        ...
        if datalab_result:
            ...
            if self._is_confidence_acceptable(datalab_result):
                ...
                if not self.enable_fallback:
                    ...
                    logger.warning(
                        f"Datalab OCR confidence {confidence:.2f} below threshold {self.confidence_threshold}, but fallback disabled, returning result"
                    )
                    self.stats[
                        "datalab_success"
                    ] += 1  # Still count as Datalab success since we're using it
                    return datalab_result
                else:
                    # Trigger fallback
                    logger.warning(
        ...
        if datalab_result:
            logger.warning(
                "Both services had issues, returning Datalab result with low confidence"
            )
            return datalab_result
