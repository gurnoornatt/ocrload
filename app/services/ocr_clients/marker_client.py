...
class MarkerOCRClient:
    ...
    def __init__(self, api_key: Optional[str] = None, session: Optional[aiohttp.ClientSession] = None):
        ...
        """
        self.api_key = api_key or settings.DATALAB_API_KEY  # Same key for all Datalab endpoints
        ...
        self.supported_mime_types = {
            'application/pdf',
            'image/png', 'image/jpeg', 'image/webp', 'image/gif', 'image/tiff', 'image/jpg',
    ...
    def _validate_file_content(self, file_content: bytes, mime_type: str) -> None:
        ...
        if len(file_content) > self.max_file_size:
            raise MarkerOCRError(f"File size {len(file_content)} exceeds maximum {self.max_file_size}")
        ...
        try:
            async with session.post(
                ...
                if response.status == 401:
                    ...
                    raise MarkerOCRError(f"API request failed with status {response.status}: {error_text}")
                ...
                if 'request_id' not in data or 'request_check_url' not in data:
                    raise MarkerOCRError("Invalid API response - missing request tracking info")
    ...
    def _convert_page_to_ocr_format(self, page_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if text_lines:
            avg_confidence = sum(line.get('confidence', 0.8) for line in text_lines) / len(text_lines)
        else:
            avg_confidence = 0.5
    ...
    def _convert_block_to_text_line(self, block_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
