...
class DatalabOCRClient:
        ...
        async with self._request_semaphore:
            try:
                async with self.session.post(
                    self.OCR_ENDPOINT, data=form_data
                ) as response:
                    ...
                    if not response_data.get("success"):
                        ...
                        raise DatalabProcessingError(
                            f"OCR submission failed: {error_msg}"
                        )
        ...
        for attempt in range(max_polls):
            try:
                ...
                if attempt == max_polls - 1:
                    raise DatalabOCRError(
                        f"Polling failed after {max_polls} attempts: {e}"
                    )
                await asyncio.sleep(current_interval)
                current_interval = min(current_interval * 1.5, 10)

        raise DatalabTimeoutError(
            f"OCR processing timed out after {max_polls} polling attempts"
        )
    ...
    async def process_file_content(
        ...
        try:
            # Submit OCR request
            logger.info(
                f"Submitting OCR request for {filename} ({len(file_content)} bytes)"
            )
