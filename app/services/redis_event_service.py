...
class RedisEventService:
    ...
    def _setup_client(self):
        ...
        try:
            ...
            # Parse Upstash URL format (expects https:// format)
            if redis_url.startswith("https://"):
                self._redis_url = redis_url
            else:
                logger.error(
                    f"Invalid Redis URL format: {redis_url}. Expected https:// format for Upstash."
                )
                return
            ...
            logger.info(
                "Redis event service initialized successfully with Upstash REST API"
            )
    ...
    async def _execute_command(self, command: list) -> Any | None:
    ...
    async def emit_invoice_ready(self, load_id: str, driver_id: str) -> bool:
        ...
        try:
            ...
            if result is not None:
                logger.info(
                    f"Invoice ready event emitted for load {load_id}, driver {driver_id}"
                )
                return True
            else:
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            if result == "PONG":
                ...
                return {
                    "status": "unhealthy",
                    "message": f"Unexpected ping response: {result}",
                    "configured": True,
                    "timestamp": datetime.utcnow().isoformat(),
                }
